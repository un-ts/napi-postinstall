...
describe('napi-postinstall', () => {
  it('should check and prepare package', async () => {
    ...
    await expect(
      ...
      `[Error: [napi-postinstall@0.3.0] No \`napi.targets\` nor \`napi.triples.additional\` field found in \`napi-postinstall\`'s \`package.json\`. Please ensure the package is built with NAPI support.]`,
    )
  })
})
